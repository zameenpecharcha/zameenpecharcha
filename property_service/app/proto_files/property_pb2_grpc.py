# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc
import warnings

import property_pb2 as property__pb2

GRPC_GENERATED_VERSION = '1.73.1'
GRPC_VERSION = grpc.__version__
_version_not_supported = False

try:
    from grpc._utilities import first_version_is_lower
    _version_not_supported = first_version_is_lower(GRPC_VERSION, GRPC_GENERATED_VERSION)
except ImportError:
    _version_not_supported = True

if _version_not_supported:
    raise RuntimeError(
        f'The grpc package installed is at version {GRPC_VERSION},'
        + f' but the generated code in property_pb2_grpc.py depends on'
        + f' grpcio>={GRPC_GENERATED_VERSION}.'
        + f' Please upgrade your grpc module to grpcio>={GRPC_GENERATED_VERSION}'
        + f' or downgrade your generated code using grpcio-tools<={GRPC_VERSION}.'
    )


class PropertyServiceStub(object):
    """Property Service Definition
    """

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.CreateProperty = channel.unary_unary(
                '/property.PropertyService/CreateProperty',
                request_serializer=property__pb2.Property.SerializeToString,
                response_deserializer=property__pb2.PropertyResponse.FromString,
                _registered_method=True)
        self.GetProperty = channel.unary_unary(
                '/property.PropertyService/GetProperty',
                request_serializer=property__pb2.PropertyRequest.SerializeToString,
                response_deserializer=property__pb2.PropertyResponse.FromString,
                _registered_method=True)
        self.UpdateProperty = channel.unary_unary(
                '/property.PropertyService/UpdateProperty',
                request_serializer=property__pb2.Property.SerializeToString,
                response_deserializer=property__pb2.PropertyResponse.FromString,
                _registered_method=True)
        self.DeleteProperty = channel.unary_unary(
                '/property.PropertyService/DeleteProperty',
                request_serializer=property__pb2.PropertyRequest.SerializeToString,
                response_deserializer=property__pb2.PropertyResponse.FromString,
                _registered_method=True)
        self.SearchProperties = channel.unary_unary(
                '/property.PropertyService/SearchProperties',
                request_serializer=property__pb2.PropertySearchRequest.SerializeToString,
                response_deserializer=property__pb2.PropertyListResponse.FromString,
                _registered_method=True)
        self.AddPropertyDocument = channel.unary_unary(
                '/property.PropertyService/AddPropertyDocument',
                request_serializer=property__pb2.PropertyDocumentRequest.SerializeToString,
                response_deserializer=property__pb2.PropertyResponse.FromString,
                _registered_method=True)
        self.AddPropertyFeature = channel.unary_unary(
                '/property.PropertyService/AddPropertyFeature',
                request_serializer=property__pb2.PropertyFeatureRequest.SerializeToString,
                response_deserializer=property__pb2.PropertyResponse.FromString,
                _registered_method=True)
        self.AddMedia = channel.unary_unary(
                '/property.PropertyService/AddMedia',
                request_serializer=property__pb2.MediaRequest.SerializeToString,
                response_deserializer=property__pb2.PropertyResponse.FromString,
                _registered_method=True)
        self.AddUserProperty = channel.unary_unary(
                '/property.PropertyService/AddUserProperty',
                request_serializer=property__pb2.UserPropertyRequest.SerializeToString,
                response_deserializer=property__pb2.PropertyResponse.FromString,
                _registered_method=True)
        self.VerifyProperty = channel.unary_unary(
                '/property.PropertyService/VerifyProperty',
                request_serializer=property__pb2.PropertyRequest.SerializeToString,
                response_deserializer=property__pb2.PropertyResponse.FromString,
                _registered_method=True)
        self.FlagProperty = channel.unary_unary(
                '/property.PropertyService/FlagProperty',
                request_serializer=property__pb2.PropertyRequest.SerializeToString,
                response_deserializer=property__pb2.PropertyResponse.FromString,
                _registered_method=True)


class PropertyServiceServicer(object):
    """Property Service Definition
    """

    def CreateProperty(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def GetProperty(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def UpdateProperty(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def DeleteProperty(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def SearchProperties(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def AddPropertyDocument(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def AddPropertyFeature(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def AddMedia(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def AddUserProperty(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def VerifyProperty(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def FlagProperty(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_PropertyServiceServicer_to_server(servicer, server):
    rpc_method_handlers = {
            'CreateProperty': grpc.unary_unary_rpc_method_handler(
                    servicer.CreateProperty,
                    request_deserializer=property__pb2.Property.FromString,
                    response_serializer=property__pb2.PropertyResponse.SerializeToString,
            ),
            'GetProperty': grpc.unary_unary_rpc_method_handler(
                    servicer.GetProperty,
                    request_deserializer=property__pb2.PropertyRequest.FromString,
                    response_serializer=property__pb2.PropertyResponse.SerializeToString,
            ),
            'UpdateProperty': grpc.unary_unary_rpc_method_handler(
                    servicer.UpdateProperty,
                    request_deserializer=property__pb2.Property.FromString,
                    response_serializer=property__pb2.PropertyResponse.SerializeToString,
            ),
            'DeleteProperty': grpc.unary_unary_rpc_method_handler(
                    servicer.DeleteProperty,
                    request_deserializer=property__pb2.PropertyRequest.FromString,
                    response_serializer=property__pb2.PropertyResponse.SerializeToString,
            ),
            'SearchProperties': grpc.unary_unary_rpc_method_handler(
                    servicer.SearchProperties,
                    request_deserializer=property__pb2.PropertySearchRequest.FromString,
                    response_serializer=property__pb2.PropertyListResponse.SerializeToString,
            ),
            'AddPropertyDocument': grpc.unary_unary_rpc_method_handler(
                    servicer.AddPropertyDocument,
                    request_deserializer=property__pb2.PropertyDocumentRequest.FromString,
                    response_serializer=property__pb2.PropertyResponse.SerializeToString,
            ),
            'AddPropertyFeature': grpc.unary_unary_rpc_method_handler(
                    servicer.AddPropertyFeature,
                    request_deserializer=property__pb2.PropertyFeatureRequest.FromString,
                    response_serializer=property__pb2.PropertyResponse.SerializeToString,
            ),
            'AddMedia': grpc.unary_unary_rpc_method_handler(
                    servicer.AddMedia,
                    request_deserializer=property__pb2.MediaRequest.FromString,
                    response_serializer=property__pb2.PropertyResponse.SerializeToString,
            ),
            'AddUserProperty': grpc.unary_unary_rpc_method_handler(
                    servicer.AddUserProperty,
                    request_deserializer=property__pb2.UserPropertyRequest.FromString,
                    response_serializer=property__pb2.PropertyResponse.SerializeToString,
            ),
            'VerifyProperty': grpc.unary_unary_rpc_method_handler(
                    servicer.VerifyProperty,
                    request_deserializer=property__pb2.PropertyRequest.FromString,
                    response_serializer=property__pb2.PropertyResponse.SerializeToString,
            ),
            'FlagProperty': grpc.unary_unary_rpc_method_handler(
                    servicer.FlagProperty,
                    request_deserializer=property__pb2.PropertyRequest.FromString,
                    response_serializer=property__pb2.PropertyResponse.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'property.PropertyService', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))
    server.add_registered_method_handlers('property.PropertyService', rpc_method_handlers)


 # This class is part of an EXPERIMENTAL API.
class PropertyService(object):
    """Property Service Definition
    """

    @staticmethod
    def CreateProperty(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/property.PropertyService/CreateProperty',
            property__pb2.Property.SerializeToString,
            property__pb2.PropertyResponse.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def GetProperty(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/property.PropertyService/GetProperty',
            property__pb2.PropertyRequest.SerializeToString,
            property__pb2.PropertyResponse.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def UpdateProperty(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/property.PropertyService/UpdateProperty',
            property__pb2.Property.SerializeToString,
            property__pb2.PropertyResponse.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def DeleteProperty(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/property.PropertyService/DeleteProperty',
            property__pb2.PropertyRequest.SerializeToString,
            property__pb2.PropertyResponse.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def SearchProperties(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/property.PropertyService/SearchProperties',
            property__pb2.PropertySearchRequest.SerializeToString,
            property__pb2.PropertyListResponse.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def AddPropertyDocument(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/property.PropertyService/AddPropertyDocument',
            property__pb2.PropertyDocumentRequest.SerializeToString,
            property__pb2.PropertyResponse.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def AddPropertyFeature(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/property.PropertyService/AddPropertyFeature',
            property__pb2.PropertyFeatureRequest.SerializeToString,
            property__pb2.PropertyResponse.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def AddMedia(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/property.PropertyService/AddMedia',
            property__pb2.MediaRequest.SerializeToString,
            property__pb2.PropertyResponse.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def AddUserProperty(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/property.PropertyService/AddUserProperty',
            property__pb2.UserPropertyRequest.SerializeToString,
            property__pb2.PropertyResponse.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def VerifyProperty(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/property.PropertyService/VerifyProperty',
            property__pb2.PropertyRequest.SerializeToString,
            property__pb2.PropertyResponse.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def FlagProperty(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/property.PropertyService/FlagProperty',
            property__pb2.PropertyRequest.SerializeToString,
            property__pb2.PropertyResponse.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)
