...
from proto_files.trending_pb2 import TrendingPost as TrendingPostProto
from proto_files import trending_pb2, trending_pb2_grpc
...
class TrendingServiceServicer(trending_pb2_grpc.TrendingServiceServicer):
    ...
    def GetTrendingPosts(self, request, context):
        trending_posts = self.trending_service.get_trending_posts(request.limit, request.offset)
        return trending_pb2.GetTrendingResponse(
            success=True,
            message="Trending posts retrieved successfully",
            trending_posts=[TrendingPostProto(**post) for post in trending_posts]
        )
...
def serve():
    ...
    trending_pb2_grpc.add_TrendingServiceServicer_to_server(TrendingServiceServicer(), server)
