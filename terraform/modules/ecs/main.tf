...
resource "aws_ecs_cluster" "main" {
  name = "${var.environment}-zpc-cluster"
...
resource "aws_security_group" "ecs_services" {
  name_prefix = "${var.environment}-ecs-services-sg"
  vpc_id      = var.vpc_id
...
resource "aws_ecs_task_definition" "user" {
  ...
  tags = {
    Name        = "${var.environment}-user-service-task"
    Environment = var.environment
  }
}

# Task Definition for Property Service
resource "aws_ecs_task_definition" "property" {
  family                   = "${var.environment}-property-service"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = var.container_cpu
  memory                   = var.container_memory
  ...
  container_definitions = jsonencode([
    {
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = aws_cloudwatch_log_group.property.name
          awslogs-region        = data.aws_region.current.name
          awslogs-stream-prefix = "ecs"
        }
      }
  ...
  tags = {
    Name        = "${var.environment}-property-service-task"
    Environment = var.environment
  }
}

# Task Definition for Posts Service
resource "aws_ecs_task_definition" "posts" {
  family                   = "${var.environment}-posts-service"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = var.container_cpu
  memory                   = var.container_memory
  ...
  tags = {
    Name        = "${var.environment}-posts-service-task"
    Environment = var.environment
  }
}

# Task Definition for Feed Service
resource "aws_ecs_task_definition" "feed" {
  family                   = "${var.environment}-feed-service"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = var.container_cpu
  memory                   = var.container_memory
  ...
  tags = {
    Name        = "${var.environment}-feed-service-task"
    Environment = var.environment
  }
}

# Task Definition for Search Service
resource "aws_ecs_task_definition" "search" {
  family                   = "${var.environment}-search-service"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = var.container_cpu
  memory                   = var.container_memory
  ...
  tags = {
    Name        = "${var.environment}-search-service-task"
    Environment = var.environment
  }
}
...
resource "aws_ecs_task_definition" "notification" {
  ...
  tags = {
    Name        = "${var.environment}-notification-service-task"
    Environment = var.environment
  }
}

# Task Definition for Trending Service
resource "aws_ecs_task_definition" "trending" {
  family                   = "${var.environment}-trending-service"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = var.container_cpu
  memory                   = var.container_memory
  ...
  container_definitions = jsonencode([
    {
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = aws_cloudwatch_log_group.trending.name
          awslogs-region        = data.aws_region.current.name
          awslogs-stream-prefix = "ecs"
        }
      }
  ...
  tags = {
    Name        = "${var.environment}-trending-service-task"
    Environment = var.environment
  }
}
...
resource "aws_cloudwatch_log_group" "property" {
  name              = "/ecs/${var.environment}-property-service"
  retention_in_days = 30
}

resource "aws_cloudwatch_log_group" "posts" {
  name              = "/ecs/${var.environment}-posts-service"
  retention_in_days = 30
}
